...
public void sendTradeMarketResults(ItemStack item) {
    ...
    public void sendTradeMarketResults(List<ItemStack> items) {
        ...
        if (Wynnventory.isDevelopmentEnvironment()) {
            Wynnventory.info("Sending item data to DEV endpoint.");
    ...
    public TradeMarketItemPriceInfo fetchItemPrices(String itemName) {
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                Wynnventory.error("Failed to fetch item price from API: " + response.body());
                return null;
            }
        } catch (Exception e) {
            Wynnventory.error("Failed to initiate item price fetch {}", e);
            return null;
        }
    }
    ...
    private String serializeMarketItems(List<TradeMarketItem> marketItems) {
        try {
            ...
            Wynnventory.LOGGER.error("Failed to serialize market items ({})", marketItems.getFirst().getItem().getName());
//            WynnventoryMod.LOGGER.error("Failed to serialize market items ({})", marketItems.getFirst().getItem().getName(), e);
    ...
    private TradeMarketItemPriceInfo parsePriceInfoResponse(String responseBody) {
        try {
            ...
            Wynnventory.error("Failed to parse item price response {}", e);
            return null;
        }
    }
